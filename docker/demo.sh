...
PIN=1234
WORK="$(mktemp -d)"
mkdir -p "$(dirname "$cyber-atharv_HSM_TOKEN")" "$(dirname "$cyber-atharv_HSM_OBJECTS")"

line() { printf '\n\033[1m== %s ==\033[0m\n' "$1"; }

line "cyber-atharv HSM — PKCS#11 module loaded by OpenSC pkcs11-tool"
pkcs11-tool --module "$MOD" --init-token --label cyber-atharv --so-pin "$SO_PIN" >/dev/null
