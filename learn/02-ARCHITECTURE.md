...
- **The token file** (`cyber-atharv_HSM_TOKEN`, default `$HOME/.cyber-atharv-hsm-token`). A single fixed-size record holding the auth state and the wrapped master key. Written with a temp-file-then-rename so a crash mid-write cannot leave a half-record.
- **The object file** (`cyber-atharv_HSM_OBJECTS`, default `$HOME/.cyber-atharv-hsm-objects`). A variable-length record holding only *token* objects (`CKA_TOKEN = CK_TRUE`); session objects are never persisted. Sensitive attribute values are sealed; public values are stored plaintext.
...
```bash
cyber-atharv_HSM_TOKEN     # path to the token record (default: $HOME/.cyber-atharv-hsm-token)
cyber-atharv_HSM_OBJECTS   # path to the object store (default: $HOME/.cyber-atharv-hsm-objects)
```
