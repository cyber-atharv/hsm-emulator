...
```
~/.cyber-atharv-hsm-token
~/.cyber-atharv-hsm-objects
```
Solution: set `cyber-atharv_HSM_TOKEN` and `cyber-atharv_HSM_OBJECTS` before every command. The module falls back to `$HOME/.cyber-atharv-hsm-*` only when those are unset.
