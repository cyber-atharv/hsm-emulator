...
[![Cybersecurity Projects](https://img.shields.io/badge/Cybersecurity--Projects-Project%20%2333-red?style=flat&logo=github)](https://github.com/cyber-atharv/Cybersecurity-Projects/tree/main/PROJECTS/advanced/hsm-emulator)
...
```bash
export cyber-atharv_HSM_TOKEN=/tmp/hsm-token
export cyber-atharv_HSM_OBJECTS=/tmp/hsm-objects
...
```
Available slots:
Slot 0 (0x0): cyber-atharv HSM Emulator Slot 0
...
> [!NOTE]
> Plain `zig build` produces a **Debug** binary. The shipped artifact is `--release=safe` (ReleaseSafe), which keeps every undefined-behavior check live and turns it into a fail-closed trap rather than silent corruption. Set both `cyber-atharv_HSM_TOKEN` and `cyber-atharv_HSM_OBJECTS` to a scratch path for tests and tool runs, or the module falls back to `$HOME/.cyber-atharv-hsm-*`.
...
```bash
docker build -t cyber-atharv-hsm:latest .
docker run --rm cyber-atharv-hsm:latest
```
